...
@Service
public class DrugEmbeddings {

    private static final int BATCH_SIZE = 100;
    private static final long SLEEP_AFTER_EACH_BATCH_MS = 3000;
    private static final int MAX_RETRIES = 5;

    // For full dataset keep null.
    // For testing use 1000, 10000 etc.
    private static final Integer LIMIT = null;

    private final VectorStore vectorStore;
    private final JdbcTemplate jdbcTemplate;

    public DrugEmbeddings(VectorStore vectorStore, JdbcTemplate jdbcTemplate) {
    ...
    public void createEmbeddings() throws IOException, CsvValidationException, InterruptedException {
        ...
        System.out.println("================================================");
        System.out.println("STANDARD GEMINI EMBEDDING INGESTION STARTED");
        ...
        if (existingRows >= totalRows) {
            System.out.println("All rows already embedded. Nothing to do.");
            return;
        }
        ...
        try (CSVReader csvReader = new CSVReader(new InputStreamReader(resource.getInputStream()))) {
            ...
            System.out.println("================================================");
            System.out.printf(
                    "DONE! %d rows embedded | %d Gemini API calls | Total time: %s%n",
                    totalProcessed,
                    geminiRequests,
    ...
    private int getExistingRowCount() {
        try {
            Integer count = jdbcTemplate.queryForObject(
                    "SELECT COUNT(*) FROM drug_vector_store", Integer.class);
    ...
    private Document buildDocument(String[] record) {
        String text =
                "Drug name: " + value(record, 0) + ".\n" +
                        "Uses: " + value(record, 2) + ".\n" +
                        "Side effects: " + value(record, 3) + ".\n" +
                        "Substitutes: " + value(record, 4) + ".\n" +
                        "Chemical class: " + value(record, 5) + ".\n" +
                        "Habit forming: " + value(record, 6) + ".\n" +
                        "Therapeutic class: " + value(record, 7) + ".\n" +
                        "Action class: " + value(record, 8) + ".";

        Map<String, Object> metadata = new HashMap<>();
        metadata.put("drugName", value(record, 0));
        metadata.put("normalizedDrugName", value(record, 1));
        metadata.put("documentType", value(record, 9));
